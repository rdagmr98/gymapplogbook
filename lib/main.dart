...
Future<int> updateStreakCliente(
  ...
  final prevDone = rawPrev != null
      ? List<String>.from(jsonDecode(rawPrev))
      : <String>[];
  final rawCurrent = prefs.getString('streak_completed_sessions_c');
  var currentDone = rawCurrent != null
      ? List<String>.from(jsonDecode(rawCurrent))
      : <String>[];
  ...
  if (lastWeek != nowWeek) {
    if (lastWeek.isNotEmpty && totalSessionNames.isNotEmpty) {
      ...
      final prevRequired =
          prefs.getInt('streak_required_count_c') ?? totalSessionNames.length;
      streakCount = (prevDone.length >= prevRequired && prevRequired > 0)
          ? streakCount + 1
          : 0;
    }
    currentDone = [];
